...
import { projects2 } from "../constants/common";
const categories = ["All", "FinTech", "Health", "On-Demand", "Web3"];
export function WorkPage() {
  const [filter, setFilter] = useState("All");
  const filteredProjects =
    filter === "All" ? projects2 : projects2.filter((p) => p.cat === filter);
  return (
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-wrap gap-4 mb-16">
          {categories.map((cat) => (
            <button
              ...
              className={`px-6 py-2 rounded-full border transition-all ${filter === cat ? "bg-white text-black border-white" : "bg-transparent text-gray-400 border-white/20 hover:border-white"}`}
            >
              {cat}
            </button>
          ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {filteredProjects.map((project, i) => (
            <div
              key={i}
              className="group relative aspect-video bg-white/5 rounded-2xl overflow-hidden border border-white/10 hover:border-[color:var(--bright-red)] transition-colors"
            >
