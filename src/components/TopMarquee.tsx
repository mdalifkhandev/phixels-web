...
export function TopMarquee() {
  ...
  useEffect(() => {
    apiService
      ...
      .catch((err) =>
        console.error("Error fetching about content for marquee:", err),
      );
  }, []);
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 h-12 bg-[color:var(--pure-black)] border-b border-white/10 flex items-center justify-between text-sm tracking-wide text-gray-400">
      {/* Left Side: Marquee */}
        ...
        <div className="flex whitespace-nowrap animate-marquee">
          {[...services, ...services, ...services].map((service, i) => (
            <motion.div
              ...
              className="mx-4 flex items-center gap-2 cursor-pointer group/item relative"
              animate={{
            ...
            >
              <service.icon
                size={16}
                className="text-[color:var(--bright-red)] group-hover/item:scale-125 group-hover/item:rotate-12 transition-all duration-300"
              />
              ...
              {hoveredIndex === i && (
                <motion.div
                  ...
                  className="absolute inset-0 -z-10 bg-[color:var(--bright-red)]/10 rounded-lg blur-sm"
                  initial={{
                    opacity: 0,
                  }}
                  animate={{
      ...
      <div className="flex items-center gap-6 px-4 bg-[color:var(--pure-black)] z-10 h-full">
        <a
          href={contact?.fiverr || "https://www.fiverr.com/zmonir24"}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-4 hover:bg-white/5 px-4 py-2 rounded-lg transition-colors group/fiverr"
        >
        ...
        <div className="flex items-center gap-4 border-l border-white/10 pl-6">
          ...
          <a
            href={
              contact?.linkedin || "https://www.linkedin.com/company/106724193"
            }
            target="_blank"
            rel="noopener noreferrer"
            className="hover:scale-110 transition-transform duration-200"
